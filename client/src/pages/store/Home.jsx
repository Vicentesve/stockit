...
const Home = () => {
  ...
  const { warehouses } = useSelector((state) => state.store);
  ...
  return (
    <main className="mx-auto max-w-screen-2xl ">
      ...
      <div className="grid grid-flow-row-dense grid-cols-1 gap-3 p-5 mx-auto -mt-16 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 md:-mt-52 place-items-center">
        <CardCategories titleCard="Shop by Category" categories={categories} />

        <CardWarehouses titleCard="Shop by Warehouse" warehouse={warehouses} />
        ...
        {categories?.map((categorie, i) => (
          ...
          >
            <CardFeed titleCard={categorie.name} imageCard={categorie.image} />
          </Link>
        ))}
      </div>
