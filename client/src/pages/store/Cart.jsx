    ...
    <div className="flex flex-col-reverse justify-center flex-1 w-full h-full mx-auto sm:space-x-5 sm:flex-row max-w-screen-2xl">
      <div className="sm:w-[80%] h-full bg-white sm:mt-5 p-3 sm:p-5">
        <h3 className="hidden text-2xl font-semibold sm:block">
          {cart.length > 0 ? "Shopping Cart" : "Your Shopping Cart is Empty😢"}
        </h3>
        ...
        <div className="sm:mt-5">
          {cart?.length > 0 ? (
            <div>
              {cart?.map((product, i) => (
                <div key={i}>
                  <p className="hidden text-sm text-right text-gray-400 sm:block">
                    Price
                  </p>
                  <CheckOutProduct
                    id={product?._id}
      ...
      <div className="p-2 sm:w-[20%] bg-white sm:mt-5 h-fit sm:border-transparent">
        <div className="flex flex-col items-center w-full border-b border-gray-200 sm:border-transparent">
          <p>
